...
export class Componentes extends BaseEntity{
    ...
    @Length(3, 200, {message: "La capacidad debe tener entre $constraint1 y $constraint2 caracteres"})
    capacity: string

    @Column({name: "velocidad"})
    @IsString()
    @IsNotEmpty({message: "La velocidad es requerida"})
    @Length(3, 200, {message: "La velocidad debe tener entre $constraint1 y $constraint2 caracteres"})
    speed: string

    @Column({name: "otras_especificaciones"})
    @IsString()
    @IsNotEmpty({message: "Otras especificaciones son requeridas"})
    @Length(3, 200, {message: "Otras especificaciones deben tener entre $constraint1 y $constraint2 caracteres"})
    otherData: string
