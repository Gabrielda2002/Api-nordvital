...
export async function getAllIpsRemite(
  req: Request,
  res: Response,
  next: NextFunction
) {
...
export async function getIpsRemite(
  req: Request,
  res: Response,
  next: NextFunction
) {
...
export async function createIpsRemite(
  req: Request,
  res: Response,
  next: NextFunction
) {
  try {
    ...
    if (erros.length > 0) {
      ...
      return res
        .status(400)
        .json({ message: "Error creating ips Remite ", messages });
    }
...
export async function updateIpsRemite(
  req: Request,
  res: Response,
  next: NextFunction
) {
  try {
    ...
    if (errors.length > 0) {
      ...
      return res
        .status(400)
        .json({ message: "Error updating ips Remite", messages });
    }
...
export async function deleteIpsRemite(
  req: Request,
  res: Response,
  next: NextFunction
) {
...
export async function getIpsRemiteByName(
  req: Request,
  res: Response,
  next: NextFunction
) {
  try {
    ...
    if (name === "@") {
      ipsRemite = await IpsRemite.createQueryBuilder("ipsRemite")
        .limit(100)
        .getMany();
    } else {
      ipsRemite = await IpsRemite.createQueryBuilder("ipsRemite")
...
export async function updateStatusIpsRemite(
  req: Request,
  res: Response,
  next: NextFunction
) {
