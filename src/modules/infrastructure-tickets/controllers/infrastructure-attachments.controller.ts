...
export async function getInfrastructureTicketAttachments(req: Request, res: Response, next: NextFunction) {
    try {
        ...
        const ticket = await InfrastructureTicket.findOne({ where: { id: parseInt(String(ticketId)) } });
        if (!ticket) {
            return res.status(404).json({ message: "Infrastructure ticket not found" });
        }

        const queryBuilder = InfrastructureAttachment.createQueryBuilder("attachment")
            .leftJoinAndSelect("attachment.uploaderRelation", "user")
            .where("attachment.ticketId = :ticketId", { ticketId });

        if (userRole && !["1", "22", "23"].includes(String(userRole))) {
...
export async function uploadInfrastructureAttachment(req: Request, res: Response, next: NextFunction) {
    try {
        const { ticketId } = req.params;
        ...
        attachment.fileUrl = `/infrastructure-tickets/${file.filename}`;
        ...
        if (errors.length > 0) {
            const filePath = path.join(__dirname, "..", "..", "..", "uploads", "infrastructure-tickets", file.filename);
        ...
        Logger.info(`Infrastructure attachment uploaded for ticket ${ticketId} by user ${userId}`);
...
export async function deleteInfrastructureAttachment(req: Request, res: Response, next: NextFunction) {
    try {
        const { attachmentId } = req.params;
        const userId = req.user?.id;
        const userRole = req.user?.rol;

        const attachment = await InfrastructureAttachment.findOne({
        ...
        const canDelete = ["1", "22"].includes(String(userRole)) || attachment.uploadedByUserId === userId;
        if (!canDelete) {
        ...
        Logger.info(`Infrastructure attachment ${attachmentId} deleted by user ${userId}`);
...
export async function getInfrastructureAttachmentById(req: Request, res: Response, next: NextFunction) {
    try {
        const { attachmentId } = req.params;
        const userRole = req.user?.rol;
        ...
        if (attachment.isInternal && !["1", "22", "23"].includes(String(userRole))) {
...
export async function generateInfrastructureAttachmentDownloadToken(req: Request, res: Response, next: NextFunction) {
    try {
        ...
        const attachmentExist = await InfrastructureAttachment.findOne({ where: { id: attachmentId } });
        if (!attachmentExist) {
            return res.status(404).json({ message: "Attachment not found" });
        }
...
export async function downloadInfrastructureAttachment(req: Request, res: Response, next: NextFunction) {
    try {
        ...
        const attachment = await InfrastructureAttachment.findOne({ where: { id: fileId } });
        if (!attachment) {
            return res.status(404).json({ message: "Attachment not found" });
        }
